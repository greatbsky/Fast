...
public class NginxStream {
	 ...
	 * @return
	 */
	public static StreamingQuery startToMongo(SparkSession spark, String servers, String topicsIn, String mongoOutputUri, String checkpointLocation) {
		Dataset<Row> source = SparkUtil.readKafka(spark, servers, topicsIn);
		...
		StreamingQuery query = counts.writeStream().option("checkpointLocation", checkpointLocation + DateUtil.getDate())
				.outputMode("update").foreach(new MongoWriter<Row>(mongoOutputUri) {
					...
					public void process(Row item) {
						System.out.println("processing: " + item.get(0));
						...
						connector.withCollectionDo(writeConfig, Document.class, collection -> {
							collection.deleteMany(new Document("key", item.get(0)));
							collection.insertOne(new Document().append("key", item.get(0)).append("value", item.get(1)));
							return true;
						});
					}
				}).start();
